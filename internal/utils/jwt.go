...
func GenerateToken(userID uint, secret string, expiresInMinutes int) (string, error) {
	...
	expirationTime := time.Now().Add(time.Duration(expiresInMinutes) * time.Minute)
...
func ValidateToken(tokenString string, secret string) (*Claims, error) {
	...
	token, err := jwt.ParseWithClaims(tokenString, claims, func(token *jwt.Token) (interface{}, error) {
		...
		// Burada backend’de sakladığın secret key’i döndürür
	...
	if !ok {
		return nil, fmt.Errorf("Claims couldn't be extreacted!")
	}
	return claims, nil
}
